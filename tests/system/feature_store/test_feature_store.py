...
class TestFeatureStore(TestMLRunSystem):
    ...
    @pytest.mark.parametrize(
        "column_name,mapping, type_description, engine",
        [
            ("bad", {69: 0}, "int", "storey"),
            ("hr_is_error", {False: "0", True: "1"}, "bool", "storey"),
            ("none_column", {None: 0}, "none", "storey"),
            (
                "department",
    ...
    def test_ingest_mapvalues_remote_by_type(
        self, column_name: str, mapping: dict, type_description: str, engine: str
    ):
        local_source_path = str(self.assets_path / "testdata_with_none.parquet")
        df = pd.read_parquet(local_source_path)
        source_path = "v3io:///projects/testdata.parquet"
        ...
        full_mapping = {column_name: mapping}
        ...
        target_path = f"v3io:///bigdata/result{uuid.uuid4()}"
        targets = [ParquetTarget(name="my_parquet", path=target_path)]
        fstore.ingest(
            featureset=measurements,
            source=ParquetSource("my_source", path=source_path),
            targets=targets,
            run_config=fstore.RunConfig(local=False),
        )
        result_path = measurements.get_target_path()
        result = pd.read_parquet(result_path)
        result = result[column_name]
        if all(result.isna()):
            raise AssertionError("The result is empty, so no replacement has occurred.")
        elif any(key in result.values for key in mapping.keys()):
